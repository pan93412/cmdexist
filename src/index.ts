...
async function EnsureFileExist(filePath: string): Promise<void> {
    ...
    if (!(stat.mode & (R_OK | X_OK))) throw new Error(`${filePath} is not executable`);
}
...
export async function CheckIfAllExist<T extends readonly string[]>(exeNames: T): Promise<[boolean, ExeList<T>]> {
